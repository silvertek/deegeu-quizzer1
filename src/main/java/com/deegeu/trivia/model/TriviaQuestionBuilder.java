...
public class TriviaQuestionBuilder {
    ...
    public TriviaQuestion build() {
        if ((id < 0) || ("".equals(this.question) || this.question.trim().isEmpty())
                ...
                || ("".equals(this.hint) || this.hint.trim().isEmpty()) || (this.lastUpdated == null)) {
            throw new IllegalStateException("TriviaQuestion builder not in buildable state.");
        }

        return new TriviaQuestion(this.id, this.question, this.answerA, this.answerB, this.answerC, this.answerD,
                this.correctAnswer, this.hint, this.lastUpdated);
    }
     ...
     * 
     * @param lastUpdated the date the question was last updated for the
     *                    {@link TriviaQuestion}
     * @return a {@link TriviaQuestionBuilder} instance
     */
